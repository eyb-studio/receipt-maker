...
import { useCompany, useManReceipts } from "@/lib/storage"
import { useT } from "@/i18n/LanguageProvider"
import { ManReceiptTemplate } from "@/components/receipt/ManReceiptTemplate"
import { ConfirmDialog } from "@/components/ConfirmDialog"
import { exportAsImage, exportAsPdf } from "@/lib/exporters"

export function ManReceiptViewPage() {
  const t = useT()
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { manReceipts, deleteManReceipt } = useManReceipts()
  const { company } = useCompany()
  const manReceipt = manReceipts.find((m) => m.id === id)
  ...
  if (!manReceipt) {
    return (
      <div className="py-16 text-center">
        <p className="text-muted-foreground">{t.manreceipts.empty}</p>
        <Button asChild className="mt-4">
          <Link to="/manreceipts">{t.actions.back}</Link>
        </Button>
      </div>
    )
  }

  const filename = `man-receipt-${manReceipt.number}`
    ...
    <>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" size="icon" asChild aria-label={t.actions.edit}>
            <Link to={`/manreceipts/${manReceipt.id}/edit`}>
      ...
      <div className="overflow-x-auto rounded-lg border bg-neutral-100 p-4 sm:p-8 dark:bg-neutral-900 print:overflow-visible print:border-0 print:bg-transparent print:p-0">
        <div className="mx-auto w-fit rounded-md shadow-lg print:shadow-none">
          <ManReceiptTemplate ref={ref} manReceipt={manReceipt} company={company} />
        </div>
      </div>
      ...
      <ConfirmDialog
        ...
        title={t.manreceipts.deleteConfirm}
        destructive
        onConfirm={() => {
          deleteManReceipt(manReceipt.id)
          toast.success(t.common.deleted)
          navigate("/manreceipts")
        }}
      />
    </>
  )
